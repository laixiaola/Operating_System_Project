...
{
    long long total_child_cost=0;	//总耗时（共享）
    sem_t* msem;	//信号量
    int shm_fd;	//共享内存
    ...
    //初始化信号量与共享内存
    msem=sem_open("mutex",O_CREAT,0666,1);
    shm_fd=shm_open("timecost",O_RDWR|O_CREAT,0666);
    ftruncate(shm_fd,sizeof(long long));
    void * mPtr=mmap(NULL,sizeof(long long),PROT_READ|PROT_WRITE,MAP_SHARED,shm_fd,0);
    *(long long*)mPtr=total_child_cost;
	...
	if(pid<0) {
		...
		c_start=get_time();
        	...
        	char pname[50]="子进程";
		sprintf(pname+strlen(pname),"%d",getpid());		
		...
		(void)close(listenfd);
        	...
        	sem_wait(msem);
        	*(long long *)mPtr+=tcost;
        	total_child_cost=*(long long *)mPtr;
        	sem_post(msem);
