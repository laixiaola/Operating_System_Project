...
struct threadpool *initThreadPool(int num_threads){
	...
	pool->stat.total_work_time_us = 0;
	pool->stat.total_wait_time_us = 0;
	pool->stat.work_count = 0;
	pool->stat.max_working = 0;
	pool->stat.min_working =num_threads;
	pool->stat.working_sum = 0;
	pool->stat.sample_count = 0;
	pool->stat.max_queue_len=0;
	pool->stat.queue_sample_count=0;
	pool->stat.queue_len_sum=0;
...
void* thread_do(struct thread* pthread){
	...
	while(pool->is_alive){
		gettimeofday(&wait_begin,NULL);

		staconvWait(pool->queue.has_jobs);

		gettimeofday(&wait_end,NULL);
		long long wait_us=(wait_end.tv_sec-wait_begin.tv_sec)*1000000LL+(wait_end.tv_usec-wait_begin.tv_usec);

		pthread_mutex_lock(&pool->thcount_lock);
		pool->stat.total_wait_time_us+=wait_us;
		pthread_mutex_unlock(&pool->thcount_lock);
		...
		if(pool->is_alive){
			...
			pthread_mutex_lock(&pool->thcount_lock);
			pool->num_working++;
			if (pool->num_working >pool->stat.max_working)
			{
				pool->stat.max_working = pool->num_working;
				pool->stat.working_sum +=pool->num_working;

				pool->stat.sample_count++;
			}
			pthread_mutex_unlock(&pool->thcount_lock);
			...
			if(curtask){
				...
				struct timeval work_begin;
				struct timeval work_end;

				gettimeofday(&work_begin,NULL);
				
				//执行任务
				func(arg);

				gettimeofday(&work_end,NULL);
				long long work_us=(work_end.tv_sec-work_begin.tv_sec)*1000000LL+(work_end.tv_usec-work_begin.tv_usec);

				pthread_mutex_lock(&pool->thcount_lock);
				pool->stat.total_work_time_us += work_us;
				pool->stat.work_count++;
				pthread_mutex_unlock(&pool->thcount_lock);
				//释放任务
				free(curtask);
			}
			...
			pthread_mutex_lock(&pool->thcount_lock);
			pool->num_working--;
			if (pool->num_working <pool->stat.min_working){
				pool->stat.min_working =pool->num_working;
			}
			if(!pool->num_working){
...
void push_taskqueue(taskqueue* queue,task* curtask){
	...
	if (pool)
	{
		if (queue->len > pool->stat.max_queue_len)
		{
			pool->stat.max_queue_len = queue->len;
		}

		pool->stat.queue_len_sum += queue->len;
		pool->stat.queue_sample_count++;
	}
